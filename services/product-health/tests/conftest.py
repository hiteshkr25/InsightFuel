...
def query_client_mock_fixture():
  async def mock_get(path: str, params: dict):
    if "activity" in path:
      return [
        {"timestamp": "2026-07-13", "dau": 150, "wau": 500, "mau": 1500, "new_users": 50, "returning_users": 100}
      ]
    elif "sessions" in path:
      return [
        {"session_id": "session_1", "duration": 120.0, "quality_score": 8.0},
        {"session_id": "session_2", "duration": 180.0, "quality_score": 7.0}
      ]
    elif "performance" in path:
      return [
        {"timestamp": "2026-07-13", "avg_load_time": 2.1, "error_count": 2}
      ]
    return []
...
def feature_intel_mock_fixture():
  async def mock_get(path: str, params: dict):
    if "rankings" in path:
      return [
        {"feature_id": "btn_checkout", "adoption_rate": 0.25, "importance_score": 1.5}
      ]
    return []
